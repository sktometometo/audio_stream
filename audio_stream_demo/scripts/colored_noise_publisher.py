...
def generateColoredNoise( std, max_range, f0, duration ):
    return np.clip( cn.powelaw_psd_gaussian( std, f0 * duration ), -max_range, max_range )
...
def main():
    ...
    A = 0.1 * ((2**8) ** 2)
    buffer_wave = generateColoredNoise( 0.8 * A, A, msg_info.sampling_rate, 30 ).astype(np.uint16).tobytes()
