...
def generateWhiteNoise( std, max_range, sampling_rate, duration ):

    return np.clip( np.array( [ random.gauss( 0.0, std ) for i in range( sampling_rate * duration ) ] ), -max_range, max_range )
...
def main():
    ...
    use_audio_common = int( rospy.get_param( '~use_audio_common', False ) )
